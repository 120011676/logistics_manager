		...
		<tr>
			...
			<td valign="top">
				<div id="content-table-inner">
					<form id="myForm" action="${path }/all/list.htm" method="post">
									...
									<td><div style="margin-left: 10px;">
											<c:if test="${c.enable == true }">
												<a class="icon-2 info-tooltip" title="删除"
													href="${path }/all/delete.htm?id=${c.id}"></a>
											</c:if>
											<c:if test="${c.enable == false }">
												<a class="icon-5 info-tooltip" title="恢复"
													style="margin: 0 8px 0 0;"
													href="${path }/all/recovery.htm?id=${c.id}"></a>
											</c:if>
											<a class="icon-3 info-tooltip" title="查看"
												href="${path }/all/look.htm?id=${c.id}"></a>
										</div></td>
								</tr>
							</c:forEach>
						</table>
